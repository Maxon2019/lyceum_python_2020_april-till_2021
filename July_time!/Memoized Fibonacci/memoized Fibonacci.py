...
This algorithm serves well its
educative purpose but it's tremendously inefficient, not only because of recursion, but because we invoke the
...
This algorithm is so inefficient that the time to calculate any Fibonacci number over 50 is simply too much. You may
go for a cup of coffee or go take a nap while you wait for the answer. But if you try it here in Code Wars you will
...
def fibonacci(n):
    ...
    if n in [0, 1]:
        ...
        ind = 3
        while ind <= n:
            m = mem_list[ind - 1] + mem_list[ind - 2]
            mem_list.insert(ind, m)
            ind += 1
        mem_list.reverse()
        return mem_list[0]
