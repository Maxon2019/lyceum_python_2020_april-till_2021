...
def snail(snail_map):
    ...
    while row_begin <= row_end and col_begin <= col_end:
        for i in range(col_begin, col_end + 1):
            res.append(snail_map[row_begin][i])  # проходимся по ряду
        row_begin += 1

        for i in range(row_begin, row_end + 1):
            res.append(snail_map[i][col_end])  # проходимся по колонне
        col_end -= 1

        if row_begin <= row_end:
            for i in range(col_end, col_begin - 1, -1):
                res.append(snail_map[row_end][i])  # меняем ряд
        row_end -= 1

        if col_begin <= col_end:
            for i in range(row_end, row_begin - 1, -1):
                res.append(snail_map[i][col_begin])  # меняем колонну
        col_begin += 1
    return res
