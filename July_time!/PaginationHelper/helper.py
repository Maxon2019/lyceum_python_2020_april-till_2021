...
helper.page_index(-10) # should == -1 because negative indexes are invali
"""
...
class PaginationHelper:
    ...
    def page_count(self):
        page = 0
        counter = len(self.collection)
        self.page = page
        while counter > 0:
            counter -= self.items_per_page
            page += 1
        # returns the number of pages
        return page
    ...
    def page_item_count(self, page_indexx):
        self.page_indexx = page_indexx
        if self.item_count() - self.items_per_page * self.page_indexx > self.items_per_page:
            return self.items_per_page
        elif self.item_count() - self.items_per_page * self.page_indexx > 0:
            return self.item_count() - self.items_per_page * self.page_indexx
        else:
            return -1
    ...
    def page_index(self, item_index):
        ...
        if self.item_index >= self.item_count():
            ...
            if self.item_index % self.items_per_page != 0:
                return self.item_index // self.items_per_page
            else:
                return self.item_index // self.items_per_page
...
if __name__ == '__main__':
    ...
    print(f'{helper.page_item_count(0)}' + ' items')  # should == 4
    print(f'{helper.page_item_count(1)}' + ' items')  # last page - should == 2
    print(f'{helper.page_item_count(2)}' + ' items')  # should == -1 since the page is invalid
